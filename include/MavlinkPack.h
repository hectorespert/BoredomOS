...
// message. src/link.cpp calls this and nothing else that names a msgid or
...
// what a msgid is."
//
// `chan` is the port the message is about to leave by, and it is what gives
// each port its own sequence numbering: the library keeps current_tx_seq per
// channel, so packing on the wrong one makes two ground stations share a
// counter and each see gaps.
//
