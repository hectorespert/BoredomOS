...
**Two things about this suite that are easy to lose an hour to.** First, **`pio test` hides `TEST_MESSAGE` output** — it prints only the `[PASSED]`/`[FAILED]` lines, so the report-only cases (`test_report_r64cnt_range`, `test_report_internal_versus_ds1307_drift`, `test_report_sd_volume_geometry`) show nothing through it. Use **`pio test -e libs -v`**, which echoes the `INFO:` lines those cases exist to produce. Second, **check `custom_mode` before a series of reflashes, not after.** Every flash is a boot, and boots that do not survive the five-minute stability window drive the consecutive fault counter towards 3, at which point the board latches the reduced configuration — which then makes `check_telemetry.py`'s `test_battery_status_every_2s` fail rather than self-skip, and the failure looks like a regression in whatever you were working on. `MAV_CMD_PREFLIGHT_REBOOT_SHUTDOWN` clears both counters and restores the normal configuration at once.
