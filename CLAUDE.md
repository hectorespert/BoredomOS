...
- **`test/test_hil/`** — host-side Python driving the flashed firmware over the MAVLink link, via `test/test_hil/run.py`. Nine cases across four `check_*.py` modules, all of them about the `mavlink-link` capability. Which scenario each case covers is **not recorded** — see *Record which scenario each HIL case covers* in [TODO.md](TODO.md). Needs `pymavlink` (`pip install -r test/test_hil/requirements.txt`). The default target is USB, which the flight build always answers, so no adapter is needed; `HIL_PORT=uart` targets D0/D1 when one is attached. `run.py --list` and `--filter` run a single case by name.
