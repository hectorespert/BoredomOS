...
- [ ] 1.1 Add `-D configSUPPORT_STATIC_ALLOCATION=1` to `build_flags` in
...
- [ ] 2.2 Implement `vApplicationMallocFailedHook()` in `src/hooks.cpp` so it calls
...
- [ ] 2.3 Choose a blink pattern distinguishable from the stack-overflow hook's, and
...
- [ ] 4.3 Recompute the worst-case heap occupancy from the code rather than from the
...
- [ ] 5.5 Verify `pio test -e libs --without-uploading --without-testing` still builds
  the Unity binary — it links no FreeRTOS objects at all, so it is unaffected, but it
  is the case `pio run -e libs` does not cover
- [ ] 5.6 Verify the RAM figure `pio run` prints has risen from 16828 bytes to roughly
...
- [ ] 5.7 Verify `openspec validate use-static-allocation --strict` and `pio check`
...
- [ ] 6.2 Update `ARCHITECTURE.md` §4 to state what backs each queue depth, including
  the in-flight allowance, and that a saturated queue is now reported by `xQueueSend`
  rather than at the point of allocation
- [ ] 6.3 Update `ARCHITECTURE.md` §7, which states the 8 KB heap and the
...
- [ ] 6.4 Update the `CLAUDE.md` conventions: stack sizes are still words but now name
...
- [ ] 6.5 Update the `context` block and the `rules.proposal` entry in
...
- [ ] 6.6 Delete the `TODO.md` entry *The two serial queues cannot fit in the FreeRTOS
...
- [ ] 6.7 Add a `TODO.md` entry for recovering the board, so the blocked steps in group
  7 point at something that is tracked, and reference it from this change
- [ ] 6.8 Re-point the two unapplied changes at the static model: `add-console-cli`
